...
describe('Library', () => {
  ...
  it('should add a book', async () => {
    await library.addBook({ id: '1234567890', title: 'Test Book', author: 'Author Name', publicationYear: 2023 });
  ...
  it('should borrow a book', async () => {
    await library.addBook({ id: '0987654321', title: 'Another Test Book', author: 'Another Author', publicationYear: 2023 });
  ...
  it('should raise an error when borrowing an unavailable book', async () => {
    ...
    await library.borrowBook('1234567890');
    await expect(library.borrowBook('1234567890')).rejects.toThrow('Book is not available for borrowing');
  });

  it('should view available books', async () => {
    // Ensure the first book is still available after borrowing the second
    const books = await library.viewAvailableBooks();
    expect(books.length).toBe(1); // Only one book should be available
  });
});
